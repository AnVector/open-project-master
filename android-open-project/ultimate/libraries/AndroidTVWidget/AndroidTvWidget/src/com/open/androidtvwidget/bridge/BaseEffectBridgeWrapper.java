...
public class BaseEffectBridgeWrapper extends BaseEffectBridge {
	...
	private static final float DEFUALT_SCALE = 1.0f;//默认放大倍数
	private static final int DEFUALT_TRAN_DUR_ANIM = 200;//默认duration

	private MainUpView mMainUpView;//最上层移动的View
	private Drawable mDrawableShadow;//边框阴影
	private Drawable mDrawableUpRect;//边框图片
	...
	public void onOldFocusView(View oldFocusView, float scaleX, float scaleY) {
		if (oldFocusView != null) {
			ObjectAnimator anim1 = ObjectAnimator.ofFloat(oldFocusView, "scaleX",scaleX);
			ObjectAnimator anim2 = ObjectAnimator.ofFloat(oldFocusView, "scaleY", scaleY);
			AnimatorSet animSet = new AnimatorSet();
			animSet.setDuration(DEFUALT_TRAN_DUR_ANIM);
...
//			oldFocusView.animate().scaleX(scaleX).scaleY(scaleY).setDuration(DEFUALT_TRAN_DUR_ANIM).start();
	...
	public void onFocusView(View focusView, float scaleX, float scaleY) {
		if (focusView != null) {
			ObjectAnimator anim1 = ObjectAnimator.ofFloat(focusView, "scaleX",scaleX);
			ObjectAnimator anim2 = ObjectAnimator.ofFloat(focusView, "scaleY", scaleY);
			AnimatorSet animSet = new AnimatorSet();
			animSet.setDuration(DEFUALT_TRAN_DUR_ANIM);
			animSet.setInterpolator(new LinearInterpolator());
			//两个动画同时执行
