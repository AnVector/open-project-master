...
public class AsyncHttpClient {
    ...
    public static final int DEFAULT_SOCKET_TIMEOUT = 1000 * 1000;
    ...
    public void cancelRequestsWithName(final Context context, final String requestCancelName, final boolean mayInterruptIfRunning) {
        if (context == null) {
            Log.e(LOG_TAG, "Passed null Context to cancelRequests");
            return;
        }
        if (requestCancelName == null || requestCancelName.equals("")) {
        	Log.e(LOG_TAG, "Passed empty string to requestCancelName");
        	return;
        }
        Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                if (requestList != null) {
                    for (RequestHandle requestHandle : requestList) {
                    	if (requestCancelName.equals(requestHandle.getRequestCancelName())) {
                    		requestHandle.cancel(mayInterruptIfRunning);
                    	}
                    }
                }
    ...
    public RequestHandle post(Context context, String url, Header[] headers, HttpEntity entity, String contentType,
        ...
        for(int i=0;i<headers.length;i++){
        	Log.d("request header"+"["+i+"]",headers[i] .toString());
        }
        Log.d ("request","url:"+url);
		Log.d("request","ContentType():"+entity.getContentType());
		Log.d("request","ContentLength():"+entity.getContentLength());
		try {
			Log.d("request","content:"+	EntityUtils.toString(entity));
